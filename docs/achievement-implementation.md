...
## 2026-09-20 AI 검토 후 조회·응답 범위 수정

Worker와 본체 reader가 공통 코드 비교 함수를 사용합니다. 균형 잡힌 바깥 대괄호와 공백 차이만 정규화하며 원문의 `raw` 코드·근거는 그대로 보존합니다. 과목, 학년군 숫자, 과목 번호, 하이픈, 괄호는 바꾸지 않습니다. 코드 필터가 있는 cursor도 같은 표기 정규화를 사용하므로 이어 읽기 도중 대괄호 유무를 바꿀 수 있습니다. 이전 코드 필터 cursor는 새 비교 키와 다를 수 있어 첨부 첫 응답부터 다시 읽어야 합니다.

profile `1.2.2`부터 알파벳 라벨은 `labelSystem: "alphabetic"`을 반환합니다. 본체는 이전 Worker의 `abc`도 받아 `alphabetic`으로 반환하며 `rawLabel`과 근거는 바꾸지 않습니다. 이전 스키마에 값 목록을 고정한 클라이언트는 새 값을 허용해야 합니다.

`levelCoverage`는 선택한 첨부와 현재 필터에서 추출된 레코드의 전달 범위를 표시합니다. `extractedLabels`는 원문 라벨의 합집합으로 최대 20개, 각 100자까지 제공하며 생략이 있으면 `labelsTruncated`가 참입니다. 여러 코드의 결과를 읽었다면 이 합집합을 각 코드의 수준 체계로 해석할 수 없습니다. `matchingRecordCount`, `returnedRecordCount`, `remainingRecordCount`, `omittedRecordCount`는 각각 전체 일치 레코드, 현재 응답, cursor 뒤에 남은 레코드, 20,000자 초과 레코드 수입니다. 남은 수에는 아직 cursor가 지나지 않은 초과 레코드도 포함됩니다.

`allMatchingRecordsDelivered`는 이전 cursor 응답까지 합쳐 추출 레코드를 전달했는지만 뜻합니다. 일치 레코드가 없거나 크기 제한으로 제외된 레코드가 있으면 거짓입니다. `sourceCompleteness`는 `unverified`이며 텍스트 추출 완료와 원문 수준 묶음의 완전성을 혼동하지 않습니다. `levelLabelFilterApplied`가 참이면 특정 라벨만의 범위입니다. `LEVEL_RECORDS_REMAIN`, `LEVEL_LABEL_FILTER_APPLIED`, `DOCUMENT_EXTRACTION_INCOMPLETE` 경고가 필요한 후속 읽기나 부분 답변 범위를 안내합니다.

도구 설명과 서버 안내는 요청 코드를 읽기 필터로 전달하고, 다른 코드의 결과를 사용자 코드의 정정으로 취급하지 않도록 명시합니다. 코드 앞의 숫자로 개정 연도를 추측하지 않고 문서의 학년군·과목·버전 근거를 확인하도록 안내합니다. 이 수정은 모델 답변 정확도 향상을 입증하는 재평가나 운영 배포를 의미하지 않습니다.

검증: `npm run check`의 타입 검사·빌드 및 236개 테스트, 별도 성취수준 golden 8/8건이 통과했습니다. 원래 평가의 `primary/haiku/D17-code/audit/9-response.json`에 보존된 2개 레코드를 Worker와 reader에 다시 넣었을 때 `[9정01-01]`, `9정01-01`, 대괄호 안팎 공백 표기는 모두 같은 2건을 반환하고 `[9과01-01]`은 0건을 반환했습니다. 저장된 일부 응답만 사용한 오프라인 조회 검증이며 그 문서의 전체 수준 수를 검증한 것은 아닙니다. 기존 평가 보고서·근거 파일은 수정하지 않았습니다.

## 2026-09-20 수정 후보 `1.1.0-rc.4`
...
profile `1.2.1`과 PDF 선 복원은 성취기준 열 및 설명 열의 병합을 각각 확인합니다. A/B 또는 C/D 공동 설명은 실제 선으로 범위가 검증된 경우에만 동일 설명·원래 근거 위치를 사용합니다. 수준 라벨과 설명이 없는 기준만 추출된 항목은 검증된 성취수준 목록에서 제외하며, 수준을 확정할 수 없으면 원문 블록으로 제공합니다. 원문 기준 자체를 삭제하거나 만들어 채우지 않습니다.

RC4는 한국사 HWP에서 확인된 `성취기준별 성취수준 진술` 머리글을 추가로 인식합니다. 정확한 문구와 parser가 제공한 2열 병합, 성취기준의 명시적 행 병합이 있어야 하며, PDF에서는 실제 표 선 검증도 유지합니다. 임의의 머리글 접미사나 비어 있는 칸으로 표 관계를 추정하지 않습니다.
...
학년·과목·수준 읽기 필터는 `raw` 또는 `normalized`와 정확히 일치하는 레코드에 적용한다. 코드 필터는 위의 표기 정규화 후 원문 `raw`와 비교한다. 요청값을 원문 필드에 채워 넣지 않는다. 수준 라벨은 A~E, 상/중/하, 서술형을 그대로 보존한다. 순서가 원문에서 명시되지 않으면 `ordinal`을 만들지 않는다. 병합 셀은 parser가 제공한 span만 사용하고 상속을 경고한다. 불명확한 페이지를 추정하지 않는다.
