...
별도 세션에서 채점기 1.1.0 수정과 전체 재실행을 수행한 결과는 [독립 실행 보고서](../../docs/achievement-product-independent-eval-2026-09-18.md)에 있다. 검수된 실문서 gold가 없는 상태는 그대로 미측정으로 남긴다.
...
채점기 `1.1.0`부터 완전한 근거 점수는 위치 키와 값까지 gold와 일치할 때만 부여한다. 예를 들어 gold가 `{page: 1}`만 확인했는데 parser가 `{page: 1, row: 999}` 또는 더 상세한 block·문자 범위를 반환하면, 추가 좌표는 **미검증**이다. 이를 곧바로 잘못된 좌표라고 단정하지 않으며 `evidenceCorrectness.unverifiedLocationFields`와 문서별 `evidenceUnverifiedLocationFields`, `field_evidence_additional_location_review_needed`로 분리한다. 해당 필드는 분모에 남고 완전 일치 점수를 받지 않으므로 추가 위치 검토가 필요한 상태에서 품질 합격이 나오지 않는다. 사람이 확인한 좌표 자체가 충돌하거나 인용·해시·값이 틀린 경우는 기존 근거 불일치로 기록한다. 과거 1.0.0 결과와 직접 합산하지 않는다.
...
실행의 완결 선언은 JSON boolean `completed: true`만 인정한다. 누락, `null`, 문자열 `"true"`/`"false"`, 숫자 등은 `collection_not_completed`로 남기며 `complete`와 `pass`가 될 수 없다. 이 경우 개별 관측 지표를 진단용으로 보존할 수 있지만 실행 전체의 완료를 대신하지 않는다.
