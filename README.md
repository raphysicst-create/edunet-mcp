...
읽기의 코드 필터는 `[9정01-01]`과 `9정01-01`처럼 대괄호·공백만 다른 표기를 같은 코드로 조회합니다. 다른 과목·학년군·과목 번호로 대체하지 않습니다. `levelCoverage`에서 추출된 라벨과 남은 레코드·제외된 레코드 수를 확인하세요. 이어 읽기가 끝나도 원문 전체 수준이 추출되었다는 보증은 아니며, 특정 `levelLabel`로 조회한 결과는 그 라벨에만 해당합니다. `labelSystem: "alphabetic"`은 A~E 등의 표기 분류로, 수준 개수를 뜻하지 않습니다.
